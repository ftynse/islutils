...
static isl_schedule_node *standardMarker(isl_schedule_node *Node, void *User) {
  ...
  // hard: replace on sequence(filter(band()), filter(mark(band(any()))),
  // filter(band())) easy: replace on filter(mark(band(any())))
  ...
  if (ScheduleNodeMatcher::isMatching(matcher, isl::manage_copy(Node))) {
    ...
    isl_id *id =
        isl_id_alloc(S->mustWrites.get_ctx().get(),
...
insertCopyBackForwardMarkNodes(isl_schedule_node *Node, void *User) {
  if (isl_schedule_node_get_type(Node) == isl_schedule_node_mark) {
    ...
    isl_union_map *extensionForwardNode =
        createCopyNode(1, setCopy, isl_schedule_node_copy(Node), context);
    isl_union_map *extensionBackwardNode =
        createCopyNode(0, setCopy, isl_schedule_node_copy(Node), context);
    using namespace builders;
    auto cpp_node = isl::manage(Node);

    auto insertCopyForwardNode =
        extension(isl::manage(extensionForwardNode), 1);
    auto new_node = insertCopyForwardNode.insertAt(cpp_node);
    auto insertCopyBackwardNode =
        extension(isl::manage(extensionBackwardNode), 0);
...
static isl_schedule_node *insertCopyForwardMarkNodes(isl_schedule_node *Node,
                                                     void *User) {
  if (isl_schedule_node_get_type(Node) == isl_schedule_node_mark) {

...
std::vector<PlaceholderSet<StrideCandidate, StridePattern>> DeviceContext::accessMatchers;

std::vector<UnpositionedPlaceholder<StrideCandidate, StridePattern>> DeviceContext::stridePlaceholderCollection;
...
static isl_union_map* createCopyNode(bool direction, isl_union_set* setCopy, isl_schedule_node* node, DeviceContext* context)
{
...
static isl::schedule_node transform(int i, isl_schedule_node* node, DeviceContext* context)
{
...
{
  printf("\n\n##ANNOTATIONS##\n\n");
  for(auto it = DeviceContext::annotationMap.cbegin(); it != DeviceContext::annotationMap.cend(); ++it)
  {
...
static  std::tuple<isl_union_map*, isl_union_set*, isl_multi_union_pw_aff*>
generateCopyScheduleFromSpace(DeviceContext* context, bool forward, isl_space* inputSpace)
{
  ...
  isl_union_map* input_union_map = getRangeDomainFromSpaceAccess(isl_space_copy(inputSpace));

  //isl_union_set_intersect(isl_union_set_copy(range));
  isl_union_map* intersection = isl_union_map_intersect_range(isl_union_map_copy(input_union_map), isl_union_set_copy(range));

  isl_union_set* wrapped_union_map = isl_union_map_wrap(intersection);
  isl_union_map* map_from_wrapped_union_map = isl_union_map_from_range(wrapped_union_map);

  isl_union_set* wrapped_map_from_wrapped_union_map = isl_union_map_wrap(map_from_wrapped_union_map);
  isl_union_map* union_wrapped_map_from_set = isl_union_map_from_range(wrapped_map_from_wrapped_union_map);
...
static std::vector<isl_space*> checkThirdCondition(DeviceContext* ctx, isl_schedule_node* node)
{
  ...
  for (auto am : DeviceContext::accessMatchers) {
    ...
    for (auto m : matches) {
      ...
      isl::space currCppSpace = m[DeviceContext::stridePlaceholderCollection.at(i)].candidateMapSpace_;
...
static isl::schedule_node createFirstTypeBuilder(DeviceContext* context, std::vector<isl_space*> spaceVector, isl_schedule_node* node)
{
  isl_union_set_list* listToTransfer = isl_union_set_list_alloc(context->ctx_, 0);
  isl_union_set* readSet = isl_union_map_range(isl_union_map_copy(context->s_->reads.copy()));
  isl_union_set* writeSet = isl_union_map_range(isl_union_map_copy(context->s_->mayWrites.copy()));
...
static isl_schedule_node* findAndReplaceDevice(DeviceContext* ctx, isl_schedule_node* node)
{
  // check conditions (1)
...
void runAllFlow(std::string fileName, bool computeSchedule)
{
  ...
  DeviceContext* context = new DeviceContext(&S, scop_pet, fileName, S.mustWrites.get_ctx().get());
  /* set options */
